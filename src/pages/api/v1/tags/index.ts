...
import { listTags, createTag } from "../../../../lib/services/tags/tag.service";
...
export const GET: APIRoute = async ({ url, locals }) => {
  try {
    ...
    const tags = await listTags(locals.supabase, userId, validationResult.data);
    ...
    return createErrorResponse("internal_error", "Failed to list tags", null, 500);
  }
};
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const deckBelongsToUser = await verifyDeckOwnership(locals.supabase, userId, deck_id);

    if (!deckBelongsToUser) {
      return createErrorResponse("invalid_deck", "Deck not found or access denied", { deck_id }, 400);
    }
    ...
    return createErrorResponse("internal_error", "Failed to create tag", null, 500);
  }
};
